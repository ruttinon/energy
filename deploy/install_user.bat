...
echo [INFO] Creating startup runner...
(
  echo @echo off
  echo "%TASK_EXE%" %TASK_ARGS%
) > "%INSTALL_DIR%\run_agent.bat"
copy /Y "%INSTALL_DIR%\run_agent.bat" "%STARTUP_DIR%\EnergyLinkAgent-run.bat" >nul

echo [INFO] Starting now...
call "%INSTALL_DIR%\run_agent.bat"
