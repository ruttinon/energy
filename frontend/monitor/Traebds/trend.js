  ...
  window.renderTrends = function (content) {
    ...
    ensureChart(() => {
      ...
      function ensureChartInstance() {
        ...
        // Note: The user snippet destroyed chart inside ensureChartInstance if it existed. 
        // But usually we want to return the existing one or destroy?
        // Let's stick to user snippet logic below if defined:
        // "if (chart) chart.destroy(); chart = new Chart(...)"
        // But wait, the user logic provided did: if (chart) chart.destroy().
        // I should probably follow that to ensure clean slate, OR check if canvas is valid.

        // Actually, let's look at the user snippet for ensureChartInstance again:
        /*
          function ensureChartInstance() {
            const ctx = document.getElementById("trendChart")?.getContext("2d");
            if (!ctx) return null;
        ...
        */
        // This is problematic if we call it in loop or repeatedly. 
        // Better to have: if (chart) return chart; (unless we want to destroy it)
        // But `drawHistory` calls it. If `drawHistory` is called multiple times, we might want to just update data, not destroy?
        // But `drawHistory` updates `chart.data` and `chart.config.type`. Re-creating chart is safer for type changes?
        // Chart.js can handle type changes by `config.type = newType; chart.update();`.
        // Let's modify `ensureChartInstance` to create only if null, or if canvas changed.

        // But for `trendChart` ID which is static in `buildUI`, it's stable.
        // Let's check `chart` variable scope (it's closed over in `renderTrends`).

        // Let's simply implement a check.
        if (chart) {
      ...
      async function drawRealtimeStart() {
        ...
        // FIX: Update chart type if changed
