...
const EnergyLinkLogin = () => {
  ...
  const { setIsAdminMode, projects } = useApp();
  ...
  const doLogin = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // If backend returned a single selected_project, store it.
        try {
        ...
        const finalPid = sessionStorage.getItem('selectedProject') || new URLSearchParams(window.location.search).get('pid') || '';
        navigate(finalPid ? `/app?pid=${encodeURIComponent(finalPid)}` : '/app');
      } else {
  ...
  return (
    <div className="w-full min-h-screen relative overflow-hidden flex items-center justify-center bg-transparent">
      ...
      {/* Login Card */}
      <div className="relative z-10 w-full max-w-md p-6 md:p-10 bg-white/20 backdrop-blur-xl border border-white/30 rounded-3xl shadow-2xl animate-in fade-in zoom-in-95 duration-500 mx-4">
        
        {/* Header */}
        <div className="text-center mb-8 md:mb-10">
          <div className="w-32 h-32 md:w-48 md:h-48 mx-auto mb-4 md:mb-6 relative group cursor-pointer">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-slate-800 tracking-wider font-orbitron bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">ENERGY LINK</h1>
          <p className="text-slate-500 text-xs md:text-sm mt-2 tracking-[0.3em] uppercase font-bold">Intelligent Monitoring System</p>
        </div>

        {/* Form */}
        <div className="space-y-5 md:space-y-6">
          {/* Project selector removed: login will use username+password and backend discovers project from per-project users.json */}
      ...
      {/* Version */}
      <div className="absolute bottom-4 right-6 text-white/40 text-xs font-mono backdrop-blur-sm px-2 py-1 rounded">
        v3.0.1-STABLE
      </div>
    </div>
  );
